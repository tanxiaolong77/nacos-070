...
public class HostReactor {
    ...
    public HostReactor(EventDispatcher eventDispatcher, NamingProxy serverProxy, String cacheDir,
        ...
        if (loadCacheAtStart) {//将C:\Users\dell\nacos\naming\quickStart下的本地磁盘缓存服务列表加载到serviceInfoMap中
        ...
        this.pushRecver = new PushRecver(this);//服务器消息推送接收器
    }
    ...
    public ServiceInfo processServiceJSON(String json) {
        ...
        if (oldService != null) {
            ...
            serviceInfoMap.put(serviceInfo.getKey(), serviceInfo);//将最新的服务接口数据更新至缓存中
            ...
            if (newHosts.size() > 0 || remvHosts.size() > 0 || modHosts.size() > 0) {//change过的服务和本地缓存的服务比较，如果没有变化则不会推入队列中，保证不会重复执行回调
                eventDispatcher.serviceChanged(serviceInfo);//重点：在PushRecver中接收推送后推入EventDispatcher中的队列中，回调用户watcher函数
                DiskCache.write(serviceInfo, cacheDir);//C:\Users\dell\nacos\naming\quickStart下将接口缓存写入本地磁盘中
            }
    ...
    public ServiceInfo getServiceInfo(final String serviceName, final String clusters, final String env,
        ...
        //生成服务查询的key：com.sam.service.UserService@@myCluster2
        String key = ServiceInfo.getKey(serviceName, clusters, env, allIPs);
        ...
        if (null == serviceObj) {//如果查不到则从服务端查询
            serviceObj = new ServiceInfo(serviceName, clusters, env);

            if (allIPs) {
                serviceObj.setAllIPs(allIPs);
            }
            //{com.sam.service.UserService@@myCluster=com.sam.service.UserService@@myCluster, com.sam.service.UserService=com.sam.service.UserService}
            serviceInfoMap.put(serviceObj.getKey(), serviceObj);

            updatingMap.put(serviceName, new Object());

            if (allIPs) {//如果在服务端获取接口信息时发现本地缓存中没有该接口信心时则会向服务端拿接口信息
        ...
        //无论上面的查询结果如何这里都会再次向UpdateTask中投递nacos接口获取的消息，用来确保接口的正确性
