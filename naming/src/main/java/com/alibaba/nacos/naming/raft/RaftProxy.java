...
public class RaftProxy {
    ...
    public static void proxyPostLarge(String api, String content, Map<String, String> headers) throws Exception {
        ...
        String url = "http://" + server + RunningConfig.getContextPath() + api;//http://172.16.4.73:8849/nacos/v1/ns/raft/publish

        HttpClient.HttpResult result =  HttpClient.httpPostLarge(url, headers, content);//注册domain（服务由domain以及list组成）
        if (result.code != HttpURLConnection.HTTP_OK) {
            throw new IllegalStateException("leader failed, caused by: " + result.content);
        }
    }
