...
public class InstanceController extends ApiCommands {
    ...
    public String register(HttpServletRequest request) throws Exception {
        ...
        /***
         * {"defaultCheckPort":80,"defaultPort":80,"healthChecker":{"type":"TCP"},
         * "metadata":{},"name":"TEST1","useIPPort4Check":true}
         */
        ...
        if (StringUtils.isNotEmpty(serviceJson)) {
        	...
        	//nacos.test.3,com.sam.service.UserService
            mockHttpRequest.addParameter("dom", WebUtils.required(request, "serviceName"));
        }

        // set cluster info:
        if (StringUtils.isNotEmpty(clusterJson)) {
        	//{"defaultPort":80,"healthChecker":{"type":"TCP"},"metadata":{},"name":"TEST1","defaultCheckPort":80,"useIPPort4Check":true}
            JSONObject cluster = JSON.parseObject(clusterJson);
            String clusterName = cluster.getString("name");//TEST1,myCluster
            if (StringUtils.isEmpty(clusterName)) {
                clusterName = UtilsAndCommons.DEFAULT_CLUSTER_NAME;
            }
            mockHttpRequest.addParameter("clusterName", clusterName);

            JSONObject healthChecker = cluster.getJSONObject("healthChecker");//{"type":"TCP"}
