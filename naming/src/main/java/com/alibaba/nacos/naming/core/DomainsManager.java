...
public class DomainsManager {
    ...
    /***
     * 新增客户端注册接口信息(leader向fallow同步数据多写时调用)
     * @param domName
     * 			xxService
     * @param ips
     * 			xx.xx.xx.xx:20880##.....
	...
	public void easyAddIP4Dom(String domName, List<IpAddress> ips, long timestamp, long term) throws Exception {
        ...
        List<IpAddress> currentIPs = dom.allIPs();//获取某个接口上所有的proviers
        ...
        /**
         *key值为：providers:com.fhqb.mnt.service.lottery.MLotteryServiceApi
         */
        String key = UtilsAndCommons.getIPListStoreKey(dom);
        /***
         * 新的value 
         * [{服务A接口A},{服务B接口A},{服务C接口A（新增）}]
         */
        String value = JSON.toJSONString(ipAddressMap.values());


        Datum datum = new Datum();//创建新的数据对象存储新注册的服务
        datum.key = key;
        datum.value = value;
        ...
        peer.heartbeatDueMs = RaftCore.getLeader().heartbeatDueMs;//复位心跳时间
        peer.leaderDueMs = RaftCore.getLeader().leaderDueMs;
        peer.state = RaftCore.getLeader().state;

        RaftCore.onPublish(datum, peer);//发布（存储）新的服务信息
    }
    ...
    public void easyRemvIP4Dom(String domName, List<IpAddress> ips) throws Exception {
        Lock lock = dom2LockMap.get(domName);//根据domName（接口Service）名称进行分段锁
        if (lock == null) {
        ...
        try {
            ...
            String oldJson = StringUtils.EMPTY;
            List<IpAddress> currentIPs = dom.allIPs();//[126.126.126.126:8848:unknown_1.0_false_false_myCluster]
            ...
            if (datum != null) {
                oldJson = datum.value;//oldJson为datum老的value
            }

            List<IpAddress> ipAddrs = setValid(oldJson, map);

            ipAddrs.removeAll(ips);//删除缓存中的地址[126.126.126.126:8848:unknown_1.0_true_false_myCluster]

            /***
             * 根据key拿到datum，然后根据datum拿到某个接口下所有的服务ip列表
             * 用老的ip列表减去新的ips得到新的值保存在对应的datum的value中
             */
